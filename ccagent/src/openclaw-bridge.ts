...
export class OpenClawBridge {
  ...
  private handleChatSend(id: string, params: Record<string, unknown>) {
    ...
    // Intercept /new — kill process, clear history, respawn
    if (text.trim().toLowerCase() === "/new") {
      this.handleNewSession(id);
      return;
    }
  ...
  private async handleNewSession(id: string) {
    console.log("[bridge] /new — resetting session");
    const runId = id;
    ...
    // Respond like a command — the client expects a chat final with a message
    this.sendEvent("chat", {
      runId,
      sessionKey: SESSION_KEY,
      state: "final",
  ...
  private emitToolResultEvent(
    ...
    this.sendEvent("agent", {
      ...
      data: {
        phase: "result",
        name: toolName || (toolCallId && this.toolCalls.get(toolCallId)?.name) || "unknown",
        toolCallId,
        result: resultText,
        isError,
      },
  ...
  private translateSdkMessage(msg: any) {
    ...
    switch (msg.type) {
      ...
      case "tool_result": {
        ...
        this.emitToolResultEvent(runId, toolCallId, msg.content ?? msg.result, !!msg.is_error, msg.tool_name);
        break;
      }

      case "user": {
        // In stream-json mode, tool results are often wrapped inside user echoes:
        // { type: "user", message: { content: [{ type: "tool_result", ... }] } }
        const content = msg.message?.content;
        if (!Array.isArray(content)) break;
        for (const block of content) {
          if (!block || block.type !== "tool_result") continue;
          const toolCallId = block.tool_use_id || block.toolUseId || undefined;
          this.emitToolResultEvent(
            runId,
            toolCallId,
            block.content ?? block.result,
            !!(block.is_error ?? block.isError),
            block.tool_name ?? block.toolName,
          );
        }
        break;
      }
      ...
      case "result": {
        ...
        // Build history content preserving the interleaved order of thinking/tool/text parts.
        // Each thinking block is stored separately so reconnects reproduce the same structure
        // that was streamed (thinking → tool → thinking → tool → text).
