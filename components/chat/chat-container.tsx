...
function AnimatedMessage({ children, index }: { children: React.ReactNode; index: number }) {
...
export function ChatContainer() {
  const { messages, isStreaming, streamingMessageId, sendMessage, stopStreaming } = useChatStream();
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="mx-auto flex w-full max-w-2xl flex-col gap-5 px-4 py-6 md:px-6">
          ...
          {messages.map((message, idx) => (
            <AnimatedMessage key={message.id} index={idx}>
              <MessageBubble
                message={message}
