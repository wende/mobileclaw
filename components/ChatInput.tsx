...
export const ChatInput = forwardRef<ChatInputHandle, {
  ...
  // Keep the extended composer on the same frosted-glass look as the pill —
  // the SVG lens filter is shaped for pill geometry, so use a plain blur here.
      ...
      >
        ...
        <div className={compact ? "flex min-h-12 items-center" : undefined}>
          ...
          <textarea
            ...
            rows={1}
            className={`block w-full resize-none overflow-hidden bg-transparent text-sm leading-[1.75rem] text-foreground placeholder:text-muted-foreground/50 focus:outline-none${compact ? " h-7 self-center p-0" : ""}`}
          />
          </div>
